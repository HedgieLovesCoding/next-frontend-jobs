    ...
    async function createUser(event) {
        ...
        if(response.status == 200) {
            ...
            if(res.success) {
                ...
                throw 'Sign up succeeded but login'
            }
        } else {
...
<h1 class="text-center text-xl mt-5">Account Required to Post a Job</h1>
    ...
    <form on:submit={createUser} class="w-1/3">
        ...
        <div class="form-control w-full">
            ...
            <input 
                type="email"
                name="email"
                placeholder="eggshine@example.com"
                class="input input-bordered w-full"
                required
            />
        ...
        <div class="form-control w-full mt-4">
            <SpinnerButton buttonText="Create Account" bind:this={spinnerButton} />
        </div>
    </form>
</div>